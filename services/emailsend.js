...
async function sendMail(to, type = 'request') { // RD, PR
  ...
  try {
    ...
    const subject  = type == 'request' ? "Új jelszó igénylése" : "Sikeres jelszófrssítés"
    const gmail = google.gmail({ version: 'v1', auth: client });

    const rawMessage = makeMail(to, senderEmail, subject, type);
...
function makeMail(to, from, subject, type) {//PR
  // RFC 2047 encoding for UTF-8 characters in email headers
  const encodedSubject = `=?UTF-8?B?${Buffer.from(subject).toString('base64')}?=`;
  const body = type == 'request' ? newPasswordRequestEmail(to) : newPasswordFeedbackEmail()
